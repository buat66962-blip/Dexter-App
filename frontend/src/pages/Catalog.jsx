...
export default function Catalog() {
  ...
  const { setQty, qtyOf, totalQty } = useCart();
    ...
    <div className="space-y-10 pb-24">
      <div className="max-w-2xl">
        ...
        <p className="mt-4 text-base leading-relaxed text-zinc-400">
          Pilih alat dan jumlahnya, lalu checkout satu order. Admin approve, kode pintu gudang otomatis dikirim.
        </p>
      </div>
            ...
            <section key={cat} data-testid={`category-section-${cat.toLowerCase().replace(/[^a-z0-9]/g, "-")}`} className="space-y-5">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                {list.map((item, idx) => {
                  ...
                  const habis = item.status === "MAINTENANCE";
                  return (
                      ...
                      <div className="min-w-0 flex-1">
                        <Link to={`/barang/${item.id}`} className="block">
                          ...
                          <p className="mt-1 font-mono text-xs text-zinc-500">
                            {item.item_code} · stok {item.quantity} pcs
                          </p>
                        </Link>
